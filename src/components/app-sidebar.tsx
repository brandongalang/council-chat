  ...
  return (
    <Sidebar collapsible="icon" {...props} className="border-r border-border bg-sidebar">
      ...
      <SidebarContent className="p-2 gap-4">
        ...
        <SidebarMenu>
             <div className="px-2 py-2">
                <span className="text-xs font-mono text-muted-foreground/70 uppercase tracking-widest">Archives</span>
            </div>
            {data.archives.map((item) => (
              <SidebarMenuItem key={item.name}>
                <SidebarMenuButton asChild tooltip={item.name} className="rounded-none h-10 hover:bg-sidebar-accent">
                  <a href={item.url}>
                    <item.icon className="size-4 text-muted-foreground" />
